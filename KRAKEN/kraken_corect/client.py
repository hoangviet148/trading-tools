...
class Client(object):
    ...
    def _request(self, method, request_path, params, proxy="", fake_ip= False ):
        ...
        if fake_ip == False:
            ...
            if method == c.GET:
                ...
                try:
                    response = requests.post(url, headers=header, data=params, proxies= proxies)
                    #print(f"response post === {response.json()} - {response.status_code}")
                except Exception as e:
                    #print(f"Exception {e}")
                    exit
    ...
    def _request_without_params(self, method, request_path, proxy="", fake_ip= False):
        return self._request(method, request_path, {}, proxy,fake_ip)

    def _request_with_params(self, method, request_path, params, proxy="", fake_ip= False):
        return self._request(method, request_path, params, proxy,fake_ip)
